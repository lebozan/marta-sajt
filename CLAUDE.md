...
### Page structure
- `index.html` — homepage. Leads with a **static hero** (`.hero-static`): one still image, an `<h1>`, a positioning line naming all three categories, and one CTA. It is 65vh (56vh on phones) rather than full-screen so the collection tiles sit above the fold. The carousel markup and JS are kept intact but the `#heroCarousel` element carries `hidden`, and the carousel IIFE bails out early when it is hidden — so no `/api/carousel` fetch and no timer. Remove that one attribute to bring the carousel back.
